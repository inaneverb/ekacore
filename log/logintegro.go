...
// Package creates the new Logger instance with specified package name
// and with behaviour determined by specified args.
//
// See New to understand what values you can pass as args.
func Package(packageName string, options ...interface{}) *Logger {

	// Why private apply instead of public? Read New's internal comments.
	newLogger := baseLogger.apply(options) // has derive() call

	if packageName != "" {
		newLogger.entry.setPackageName(packageName)
	}

	return newLogger
}

// Func creates the new Logger instance with specified function name
// and with behaviour determined by specified args.
//
// See New to understand what values you can pass as args.
func Func(funcName string, options ...interface{}) *Logger {

	// Why private apply instead of public? Read New's internal comments.
	newLogger := baseLogger.apply(options) // has derive() call

	if funcName != "" {
		newLogger.entry.setFuncName(funcName)
	}

	return newLogger
}

// Class creates the new Logger instance with specified class name
// and with behaviour determined by specified args.
//
// See New to understand what values you can pass as args.
func Class(className string, options ...interface{}) *Logger {

	// Why private apply instead of public? Read New's internal comments.
	newLogger := baseLogger.apply(options) // has derive() call

	if className != "" {
		newLogger.entry.setClassName(className)
	}

	return newLogger
}

// Method creates the new Logger instance with specified class and method names
// and with behaviour determined by specified args.
//
// See New to understand what values you can pass as args.
func Method(className, methodName string, options ...interface{}) *Logger {
	...
	if methodName != "" {
		newLogger.entry.setMethodName(methodName)
	}
