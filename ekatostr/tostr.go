...
const (
	...
	TOSTR_BH_LOW_ON    uint8 = 0x01 // Include parsing arrays, structs, maps
	TOSTR_BH_LOW_JSON  uint8 = 0x02 // Use JSON for arrays, structs, maps
	TOSTR_BH_HIGH_ON   uint8 = 0x04 // Include functions, channels, interfaces
	TOSTR_BH_GO_PTR    uint8 = 0x08 // Dereference pointers before
	TOSTR_BH_SKIP_ZERO uint8 = 0x80 // Ignore zero values
)
...
// If they're the same, it means that TOSTR_BH_SKIP_ZERO flag was led
// to skipping the zero value.
//
// WARNING!
// Specifying TOSTR_BH_GO_PTR allows to request follow the pointers,
...
func ToStrTo(to []byte, v any, bh uint8) ([]byte, reflect.Kind) {
	...
	if bh&TOSTR_BH_LOW_JSON != 0 {
		bh |= TOSTR_BH_LOW_ON // Fix 'bh' if it's unexpected state.
	}
...
func ToStr(v any) string {
	var disabled = TOSTR_BH_SKIP_ZERO | TOSTR_BH_LOW_JSON
