...
type (
	...
	onceInUpdater struct {
		...
		/* 8b */ ts ekatime.Timestamp // cached current Timestamp
		/* 4b */ d ekatime.Date // cached current Date
		/* 4b */ t ekatime.Time // cached current Time
		/* 4b */ cbNum uint32 // number of associated callbacks
		/* -- */ repeatDelay ekatime.Timestamp
	}
	...
	onceInExeElem struct {
		when        ekatime.Timestamp
		repeatDelay ekatime.Timestamp
		afterDelay  ekatime.Timestamp
		cb          OnceInCallback
...
func (oie onceInExeElem) Compare(anotherOie heap.Item) int {
	...
	if whenCmp := oie.when.Cmp(oie2.when); whenCmp != 0 {
		...
		return ekatime.Timestamp(oie.cbNum).Cmp(ekatime.Timestamp(oie2.cbNum))
	}
}
...
func (oie onceInExeElem) invoke(ts ekatime.Timestamp) {
...
func initOnceIn() {
	...
	now := ekatime.NewTimestampNow()

	OnceInMinute.init(now, ekatime.SECONDS_IN_MINUTE)
	OnceIn10Minutes.init(now, ekatime.SECONDS_IN_MINUTE*10)
	OnceIn15Minutes.init(now, ekatime.SECONDS_IN_MINUTE*15)
	OnceIn30Minutes.init(now, ekatime.SECONDS_IN_MINUTE*30)
	OnceInHour.init(now, ekatime.SECONDS_IN_HOUR)
	OnceIn2Hour.init(now, ekatime.SECONDS_IN_HOUR*2)
	OnceIn3Hour.init(now, ekatime.SECONDS_IN_HOUR*3)
	OnceIn6Hour.init(now, ekatime.SECONDS_IN_HOUR*6)
	OnceIn12Hours.init(now, ekatime.SECONDS_IN_HOUR*12)
	OnceInDay.init(now, ekatime.SECONDS_IN_DAY)
