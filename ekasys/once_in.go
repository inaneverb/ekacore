...
type (
	...
	OnceInCallback func(ts ekatime.Timestamp)
...
func (oiu *onceInUpdater) Now() ekatime.Timestamp {
	return ekatime.Timestamp(atomic.LoadInt64((*int64)(&oiu.ts)))
}

// Date returns the cached Date from the current onceInUpdater that caches
// the current Date once in the specified period.
func (oiu *onceInUpdater) Date() ekatime.Date {
	return ekatime.Date(atomic.LoadUint32((*uint32)(&oiu.d)))
}

// Time returns the cached Time from the current onceInUpdater that caches
// the current Time once in the specified period.
func (oiu *onceInUpdater) Time() ekatime.Time {
	return ekatime.Time(atomic.LoadUint32((*uint32)(&oiu.t)))
}
...
func (oiu *onceInUpdater) After(delayInSec ekatime.Timestamp, invokeNow bool, cb OnceInCallback, panicCb ...OnceInPanicCallback) {
